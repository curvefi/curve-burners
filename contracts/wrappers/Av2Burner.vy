...
"""
@title AToken Burner
@notice Withdraws AAVE v2 Tokens
"""
...
LENDING_POOL: immutable(LendingPool)

ETH_ADDRESS: constant(address) = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE
PROXY: public(immutable(Proxy))

is_killed: public(bool)
killed_coin: public(HashMap[aERC20, bool])
...
def __init__(_proxy: Proxy, _owner: address, _emergency_owner: address):
    ...
    LENDING_POOL = LendingPool(0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9)


@internal
def _burn(_coin: aERC20, _amount: uint256):
    """
    @notice Withdraws underlying asset
    """
    assert not self.is_killed and not self.killed_coin[_coin], "Is killed"

    underlying: address = _coin.UNDERLYING_ASSET_ADDRESS()
    LENDING_POOL.withdraw(underlying, _amount, PROXY.address)


@external
def burn(_coin: aERC20) -> bool:
    """
    @notice Unwrap `_coin`
...
@external
def burn_amount(_coin: aERC20, _amount_to_burn: uint256):
    """
...
@external
@view
def burns_to(_coin: aERC20) -> DynArray[address, 8]:
    """
    @notice Get resulting coins of burning `_coin`
    @param _coin Coin to burn
    """
    return [_coin.UNDERLYING_ASSET_ADDRESS()]
...
@external
def set_killed(_is_killed: bool, _coin: aERC20=empty(aERC20)):
    """
