...
killed_coin: public(HashMap[yERC20, bool])
...
@internal
def _burn(_coin: yERC20, _amount: uint256):
    """
    @notice Withdraws underlying token
    """
    assert not self.is_killed and not self.killed_coin[_coin], "Is killed"

    _coin.withdraw(_amount)

    underlying: ERC20 = ERC20(_coin.token())
...
@external
def burn_amount(_coin: yERC20, _amount_to_burn: uint256):
    """
...
@external
@pure
def burns_to(_coin: yERC20) -> DynArray[address, 8]:
    """
...
@external
def set_killed(_is_killed: bool, _coin: yERC20=empty(yERC20)):
    """
