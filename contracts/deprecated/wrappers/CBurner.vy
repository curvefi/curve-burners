# @version 0.3.9
"""
@title CTokens Burner
@notice Redeems Compound Tokens
"""
...
killed_coin: public(HashMap[cERC20, bool])
...
@internal
def _burn(_coin: cERC20, _amount: uint256):
    """
    @notice Redeems underlying coin
    """
    assert not self.is_killed and not self.killed_coin[_coin], "Is killed"

    assert _coin.redeem(_amount) == 0

    underlying: ERC20 = ERC20(_coin.underlying())
    assert underlying.transfer(
        PROXY.address, underlying.balanceOf(self), default_return_value=True
    )  # safe transfer


@external
def burn(_coin: cERC20) -> bool:
    """
    @notice Unwrap `_coin`
...
@external
def burn_amount(_coin: cERC20, _amount_to_burn: uint256):
    """
...
@external
@view
def burns_to(_coin: cERC20) -> DynArray[address, 8]:
    """
    @notice Get resulting coins of burning `_coin`
    @param _coin Coin to burn
    """
    return [_coin.underlying()]
...
@external
def set_killed(_is_killed: bool, _coin: cERC20=empty(cERC20)):
    """
