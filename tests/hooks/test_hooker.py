...
def hooks(bridger, target):
    ...
    return [
        (  # Mandatory
            bridger.address,
            bridger.bridge.prepare_calldata(target, ZERO_ADDRESS)[:-32],  # omit _receiver
            (0, (0, 0, 0), 0, 0, False),
            True,
        ),
        (  # Optional
            ZERO_ADDRESS,
            b"",
            (10**9, (0, 0, 1), 0, 0, False),
            False,
        ),
        (  # Time frame
            bridger.address,
            bridger.block_hash_apply.prepare_calldata(b"")[:4],  # only method_id
            (10**9, (0, 0, 1), 1000, 2000, False),
            False,
        ),
        (  # Overlap time frame
            ZERO_ADDRESS,
            b"",
            (10**9, (0, 0, 1), 2000, 1000, False),
            False,
        ),
        (  # Dutch
            ZERO_ADDRESS,
            b"",
            (10**18, (0, 0, 1), 0, 0, True),
            False,
        ),
        (  # Dutch with time frame
            bridger.address,
            bridger.distribute.prepare_calldata(),
            (10**18, (0, 0, 1), 2000, 3000, True),
            False,
        ),
        (  # Dutch with overlap time
            ZERO_ADDRESS,
            b"",
            (10**18, (0, 0, 1), 3000, 2000, True),
            False,
        ),
        (  # Some time in future
            ZERO_ADDRESS,
            b"",
            (10**3, (START_TIME // WEEK + 2, 0, 1), 0, 0, False),
            False,
        ),
...
def test_compensation(hooker, hook_inputs):
    ...
    assert hooker.calc_compensation(hook_inputs(0), False, ts + 1000) == 0, (
        "Only free mandatory call any time"
    )
    assert hooker.calc_compensation(hook_inputs(0), False, ts + 12345) == 0, (
        "Only free mandatory call any time"
    )

    assert hooker.calc_compensation(hook_inputs(1), False, ts) == 10**9, "Optional not accounted"
    assert hooker.calc_compensation(hook_inputs(1), False, ts + 12345) == 10**9, (
        "Optional not accounted"
    )

    # [1000, 2000)
    assert hooker.calc_compensation(hook_inputs(2), False, ts + 999) == 0, (
        "Time frame not started yet"
    )
    assert hooker.calc_compensation(hook_inputs(2), False, ts + 1000) == 10**9, "Time frame start"
    assert hooker.calc_compensation(hook_inputs(2), False, ts + 1999) == 10**9, (
        "Time frame not ended yet"
    )
    assert hooker.calc_compensation(hook_inputs(2), False, ts + 2000) == 0, "Time frame ended"

    # [2000, 1000)
    assert hooker.calc_compensation(hook_inputs(3), False, ts + 999) == 10**9, (
        "Overlap not ended yet"
    )
    assert hooker.calc_compensation(hook_inputs(3), False, ts + 1000) == 0, "Overlap ended"
    assert hooker.calc_compensation(hook_inputs(3), False, ts + 1999) == 0, (
        "Overlap not started yet"
    )
    ...
    # [2000, 3000)
    assert hooker.calc_compensation(hook_inputs(5), False, ts + 2000) <= 10, (
        "Dutch time frame started"
    )
    assert (
        10**18 // 2 - 10
        <= hooker.calc_compensation(hook_inputs(5), False, ts + 2500)
        <= 10**18 // 2 + 10
    ...
    assert (
        10**18 // 3
        <= hooker.calc_compensation(hook_inputs(6), False, ts + WEEK // 2)
        <= 2 * 10**18 // 3
    ...
    with boa.env.anchor():
        ...
        assert hooker.calc_compensation(hook_inputs(7), False, ts + 3 * WEEK) == 10**3, (
            "Time has come"
        )

    assert hooker.calc_compensation(hook_inputs(0, 1, 2, 3), False, ts) == 2 * 10**9, (
        "Static don't sum up"
    )
    assert hooker.calc_compensation(hook_inputs(0, 1, 2, 3), False, ts + 1000) == 2 * 10**9, (
        "Static don't sum up"
    )
    assert hooker.calc_compensation(hook_inputs(0, 1, 2, 3), False, ts + 2000) == 2 * 10**9, (
        "Static don't sum up"
    )
